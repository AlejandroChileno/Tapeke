...
class Explorador extends React.Component {
 ...
 getCategoria(icon, description, url) {
  return <>
   <SView height={30} />
   <SView height={28} flex border={'transparent'} style={{ maxWidth: '80%', paddingLeft: 10, paddingRight: 10, backgroundColor: STheme.color.card, borderRadius: 5, overflow: 'hidden' }} center
    onPress={() => {
    ...
    <SView col={"xs-12 md-12 lg-12"} row center  >
     ...
     <SView flex center height={28}  >
      <SText fontSize={14} color={!icon ? '#999999' : STheme.color.primary} font={"LondonMM"} bold > {description}</SText>
     </SView>
    </SView>
   </SView>
   <SView width={14} />
